setwd("D:/")
data = read.csv("D:/FinancialData.csv", stringsAsFactors=FALSE)
colnames(data)
head(data)
...
imputeByLinearRegression <- function(vals, lookback=12) {
  ...
  row.train = max(rows.to.impute[1] - lookback, 0): (rows.to.impute[1] -1)
...
#impute columns
for (i in c(col_monthly.inflation.rate,3:8)) {
...
col_indices = c("DJI", "DJV", "S.P", "FTSE", "FTV", "STI", "STIV", "USPRIME.weekly", "UKPRIME.weekly", "SGPRIME.weekly", "USINF.weekly", "UKINF.weekly", "SGINF.weekly", "SG.D.forward.week", "P.D.forward.week")
data[,paste0(col_indices, ".z")] = z_score(data[,col_indices])
