...
trainSaveModels <- function(model.dataset, col.target, col.input, label) {
  ...
  # Decision Tree
  # model.rpart <- rpart(as.formula(paste0(col.target," ~ .")),
  # data=model.dataset,
  # method="class",
...
#   save(model.ada, file=paste0(modelsDir, "model.ada", label, ".Rdata"))
  ...
  allModels = list(model.ksvm=model.ksvm, model.rf=model.rf)
  return(allModels)
}
...
testModels <- function(allModels, test.dataset.withoutTarget) {
  ...
  ## Add results to stock info
  #results=cbind(test.dataset.withoutTarget[,c("Date","SecId")],results)
