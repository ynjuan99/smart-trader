...
getAllDistinctDatesFactorScore <- function(startDateIncluded, endDateExcluded) {
  channel <- odbcConnect("localDB")
  cal.dates <- sqlQuery(channel,
                        paste0("Select Distinct [Date]
                               FROM [S&P].[dbo].[tb_FactorScore]
                               where [Date] >= '", startDateIncluded,
                               "' AND [Date] < '", endDateExcluded,
                               "' order by [Date]"
                        ))
  return(as.character(cal.dates[,1]))
}
# allDistinctDates = getAllDistinctDatesFactorScore('2008-01-01', '2010-01-01')
