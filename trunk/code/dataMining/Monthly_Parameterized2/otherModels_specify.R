library(caret)
source("Monthly_Parameterized2/trainAndTest.R")

static_limit_bin_negative = 1

# testStartDate = '2009-01-01'
runTrainValidateAndTest <- function(train.dataset, validate.dataset=NULL, test.dataset, label) {
...
dataset.train[dataset.train$diff20 < static_limit_bin_negative, "bin.diff20"] = 0

dataset.test$bin.diff20 = cut(dataset.test$diff20, 
                               quantile(dataset.test$diff20, c(0, 0.8, 1)), 
                               labels=c(0,1), include.lowest=TRUE)
# dataset.test[dataset.test$diff20 < static_limit_bin_negative, "bin.diff20"] = 0
actual.testResults = dataset.test[, c("Date", "SecId", "diff20", "bin.diff20")]
...
if (!is.null(validate.dataset)) {
  dataset.validate$bin.diff20 = cut(dataset.validate$diff20, 
                                    quantile(dataset.validate$diff20, c(0, 0.8, 1)), 
                                    labels=c(0,1), include.lowest=TRUE)
  actual.validationResults = dataset.validate[, c("Date", "SecId", "diff20", "bin.diff20")]
  dataset.validate$diff20 = NULL
